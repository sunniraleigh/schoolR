...
The user can pipe the function `draw_ngon` to the function `ngong` to visualize the respective polygon. The `draw_ngon` function takes in as an argument the output of `ngon`, a tibble. 
```{r}
ngon(2,3) %>%
  draw_ngon()
```
